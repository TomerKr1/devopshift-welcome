provider "aws" {
  region = "us-east-1"
}

variable "create_vpc" {
  type    = bool
  default = true
}

variable "create_ec2" {
  type    = bool
  default = true
}

variable "myname" {}





resource "aws_vpc" "custom_vpc" {
  count = var.create_vpc ? 1 : 0

  cidr_block = "10.0.0.0/16"
  enable_dns_hostnames = true
  enable_dns_support = true
  tags = {
    Name = "TomerK-vpc"
  }
}

resource "aws_subnet" "custom_subnet" {
  count = var.create_vpc ? 1 : 0

  vpc_id                  = aws_vpc.custom_vpc[0].id
  cidr_block              = "10.0.1.0/24"
  map_public_ip_on_launch = true

  tags = {
    Name = "TomerK-subnet"
  }
}

data "aws_subnets" "default" {
  filter {
    name   = "default-for-az"
    values = ["true"]
  }
}

resource "aws_instance" "example" {
  count = var.create_ec2 ? 1 : 0

  ami           = "ami-0c02fb55956c7d316" # Ubuntu AMI
  instance_type = "t2.micro"

  subnet_id = var.create_vpc ? aws_subnet.custom_subnet[0].id : data.aws_subnets.default.ids[0]

  associate_public_ip_address = true # Ensure instance gets a public IP

  tags = {
    Name = "Tomer-ec2"
  }

  depends_on = [aws_vpc.custom_vpc]
}

output "vpc_info" {
  value = var.create_vpc ? "The following is your VPC ID: ${aws_vpc.custom_vpc[0].id} and Subnet ID: ${aws_subnet.custom_subnet[0].id}" : "Using Default VPC and Subnet"
}

output "public_ip" {
  value = var.create_ec2 ? aws_instance.example[0].public_ip : "No EC2 instance created"
}
